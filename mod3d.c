...
#pragma omp for
	...
	for (iz=0; iz < nbt; iz++) {  
        ...
        for (iy=0; iy < nbly; iy++) {
	  i = nz2*nx2*iy + nz2*ix + iz;
	  if(iz>=ix && iz>=iy){a[i]*=wt[iz];}
	  else
	  {
	  if(ix>=iz && ix>=iy){a[i]*=wlx[ix];}
	  else
	  {if(iy>=iz && iy>=ix) a[i]*=wly[iy];}
	  }
        }
        }
    }
...
#pragma omp for
    ...
    for (iz=0; iz < nbt; iz++) {  
        for (ix=0; ix < nbrx; ix++) {
        for (iy=0; iy < nbry; iy++) {
	  i = nz2*nx2*(ny2-1-iy) + nz2*(nx2-1-ix) + iz;
	  if(iz>=ix && iz>=iy){a[i]*=wt[iz];}
	  else
	  {
	  if(ix>=iz && ix>=iy){a[i]*=wrx[ix];}
...
#pragma omp for
    ...
    for (iz=0; iz < nbb; iz++) {  
        for (ix=0; ix < nblx; ix++) {
        for (iy=0; iy < nbry; iy++) {
	  i = nz2*nx2*(ny2-1-iy) + nz2*ix + nz2-1-iz;
	  if(iz>=ix && iz>=iy){a[i]*=wb[iz];}
	  else
	  {
	  if(ix>=iz && ix>=iy){a[i]*=wlx[ix];}
...
#pragma omp for
    ...
    for (iz=0; iz < nbb; iz++) {  
        for (ix=0; ix < nbrx; ix++) {
        for (iy=0; iy < nbly; iy++) {
	  i = nz2*nx2*iy + nz2*(nx2-1-ix) + nz2-1-iz;
	  if(iz>=ix && iz>=iy){a[i]*=wb[iz];}
	  else
	  {
	  if(ix>=iz && ix>=iy){a[i]*=wrx[ix];}
...
#pragma omp for
    ...
    for (iz=0; iz < nbb; iz++) {  
        for (ix=0; ix < nbrx; ix++) {
        for (iy=0; iy < nbly; iy++) {
	  i = nz2*nx2*(ny2-1-iy) + nz2*(nx2-1-ix) + nz2-1-iz;
	  if(iz>=ix && iz>=iy){a[i]*=wb[iz];}
	  else
	  {
	  if(ix>=iz && ix>=iy){a[i]*=wrx[ix];}
